...
- [06-wsl-dependency-check](issues/06-wsl-dependency-check.md) — 定稿轻检启动（wsl+Ubuntu，<1s，缺→顶部黄条）+ 全检转换（venv/三模型，缺→内嵌错误卡+复制诊断+重试）；**8888 被占→随机不常用端口**（20000-32767 随机，避开 8888 顺延段；复用前校验 Hy-MT2 模型 id），端口项降为 GUI 信息性显示。**2026-09-05 补遗（0809.mp4 实测翻车后定稿）**：PREFLIGHT_SH 内联脚本经 wsl.exe 传参失真——$变量被吃空+引号错位（`[ -e $p ]` 退化 `[ -e ]` 恒真/恒假），全检假 MISS 全灭致首次点开始必报「缺少依赖文件」；且壳未合并 stderr，[ERR] 详情全丢成黑盒。定稿修法：**预检与执行脚本常驻 WSL 侧**（gui/host/preflight.sh、run_one.sh，bash 直接读文件语法不受限），GUI 仅以静态 argv 调用（无 $/引号/分号穿越）；四 QProcess 一律 MergedChannels；成功页⇔输出文件存在作为验收不变量（0809.mp4 全流程 70s 实测 PASS）
